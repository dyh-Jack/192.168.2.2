...
import pointnet2_utils_mine as pointnet2_utils
...
class P4DConv(nn.Module):
    ...
    def forward(self, xyzs: torch.Tensor, features: torch.Tensor = None) -> (torch.Tensor, torch.Tensor):
        ...
        for t in range(self.temporal_kernel_size//2, len(xyzs)-self.temporal_kernel_size//2, self.temporal_stride):                 # temporal anchor frames
            ...
            anchor_xyz_flipped = pointnet2_utils.index_points(xyzs[t], anchor_idx)                                                   # (B, N//spatial_stride, 3)
            # print(anchor_xyz_flipped.shape)
            anchor_xyz_expanded = torch.unsqueeze(anchor_xyz_flipped, 3)                                                            # (B, N//spatial_stride, 3, 1)
            anchor_xyz_expanded = anchor_xyz_expanded.transpose(1,2).contiguous()
            ...
            for i in range(t-self.temporal_kernel_size//2, t+self.temporal_kernel_size//2+1):
                ...
                if self.in_planes != 0:
                    ...
                    # print(neighbor_feature_grouped.shape)
