...
from datasets.HOI4D import *
import HOI4D as Models
...
def train_one_epoch(model, criterion, optimizer, lr_scheduler, data_loader, device, epoch, print_freq, args):
    ...
    for pc1, rgb1, label1, mask1, pc2, rgb2, label2, mask2 in metric_logger.log_every(data_loader, print_freq, header):
        ...
        cnt += 1
        if cnt % 1000 == 5:
            print("save model to: " + 'model_{}_{}.pth'.format(epoch, cnt))
            if args.output_dir:
                checkpoint = {
                ...
                utils.save_on_master(
                    checkpoint,
                    os.path.join(args.output_dir, 'model_{}_{}.pth'.format(epoch, cnt)))
                utils.save_on_master(
                    checkpoint,
...
def evaluate(model, criterion, data_loader, device, print_freq):
    ...
    with torch.no_grad():
        for pc1, rgb1, label1, mask1, pc2, rgb2, label2, mask2 in metric_logger.log_every(data_loader, print_freq, header):
            ...
            if (loss1.item()+loss2.item())/2.0 < 10.0:
                for c in range(49):
...
def parse_args():
    ...
    parser.add_argument('--data-path', default='/nas/datasets/HOI4D_4Dseg_experiments/data_prepared/', help='data path')
    parser.add_argument('--data-train', default='/nas/datasets/HOI4D_4Dseg_experiments/datalists/train_final.txt', help='meta list for training')
    parser.add_argument('--data-eval', default='/nas/datasets/HOI4D_4Dseg_experiments/datalists/test_final.txt', help='meta list for test')
    ...
    parser.add_argument('--lr', default=0.01, type=float, help='initial learning rate')  # 0.01
